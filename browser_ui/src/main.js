...
function updateViewerNotice() {
  ...
  const mode = useCanvasFallback
    ? "canvas fallback"
    : `Three.js + Rapier mode (${webglMode})`;
...
function drawFallbackGantry() {
  ...
  const drawGantryFrame = ({ x: cx, y: cy, span, dynamic = false }) => {
    ...
    const corners = [
      new THREE.Vector3(cx - pillarX, cy - pillarY, 0),
      new THREE.Vector3(cx - pillarX, cy + pillarY, 0),
      new THREE.Vector3(cx + pillarX, cy - pillarY, 0),
      new THREE.Vector3(cx + pillarX, cy + pillarY, 0),
    ];
    ...
    drawLine3d(
      new THREE.Vector3(cx - topRailLength * 0.5, cy - pillarY, GANTRY_DIMS.topZ),
      new THREE.Vector3(cx + topRailLength * 0.5, cy - pillarY, GANTRY_DIMS.topZ),
      "#7cd1ff",
      10
    );
    ...
    drawLine3d(
      new THREE.Vector3(cx - pillarX, cy - runwayLength * 0.5, GANTRY_DIMS.topZ),
      new THREE.Vector3(cx - pillarX, cy + runwayLength * 0.5, GANTRY_DIMS.topZ),
      "#91d8f4",
      9
    );
    ...
    drawLine3d(
      new THREE.Vector3(cx - pillarX, cy - pillarY, braceZLow),
      new THREE.Vector3(cx - pillarX, cy + pillarY, braceZHigh),
      "#6aaeca",
      4
    );
    drawLine3d(
    ...
    drawLine3d(
      new THREE.Vector3(cx - localTravelX, cy + bridgeYLocal, GANTRY_DIMS.bridgeZ),
      new THREE.Vector3(cx + localTravelX, cy + bridgeYLocal, GANTRY_DIMS.bridgeZ),
      "#d6ff57",
      11
    );
    ...
    for (let laneIndex = 0; laneIndex < armCount; laneIndex += 1) {
      ...
      drawLine3d(
        new THREE.Vector3(cx - localTravelX * 0.96, cy + bridgeYLocal + laneY, GANTRY_DIMS.bridgeZ + 0.07),
        new THREE.Vector3(cx + localTravelX * 0.96, cy + bridgeYLocal + laneY, GANTRY_DIMS.bridgeZ + 0.07),
        "#9ab923",
        2
      );
    ...
    for (let armIndex = 0; armIndex < armCount; armIndex += 1) {
      ...
      const armX = cx + (-armTrackHalf + lane * (armTrackHalf * 2));
      const drop = GANTRY_ARM_CONFIG.idleDrop;
      const armTop = new THREE.Vector3(armX, cy + bridgeYLocal + laneY, GANTRY_DIMS.bridgeZ - 0.02);
      const armBottom = new THREE.Vector3(armX, cy + bridgeYLocal + laneY, GANTRY_DIMS.bridgeZ - drop);
    ...
    drawLine3d(
      new THREE.Vector3(cx - localTravelX, cy + bridgeYLocal, GANTRY_DIMS.bridgeZ + 0.08),
      new THREE.Vector3(cx - localTravelX, cy + bridgeYLocal, GANTRY_DIMS.bridgeZ - 0.08),
      "#f5fbff",
      6
    );
    ...
    if (dynamic) {
      drawLine3d(
        new THREE.Vector3(cx + carriageX, cy + bridgeY, GANTRY_DIMS.bridgeZ),
        new THREE.Vector3(cx + carriageX, cy + bridgeY, hookZ),
        "#ffb366",
        6
      );
      const isCupContact =
        Math.abs((cx + carriageX) - getSourcePickupCenter().x) < 0.12 &&
        Math.abs((cy + bridgeY) - getSourcePickupCenter().y) < 0.12 &&
      ...
      const gripperPoint = project(new THREE.Vector3(cx + carriageX, cy + bridgeY, hookZ));
      if (gripperPoint) {
...
viewerCanvas.addEventListener("mousedown", (event) => {
  ...
  fallbackPointer.mode = event.button === 2 || event.shiftKey ? "pan" : "orbit";
...
window.addEventListener("mousemove", (event) => {
  ...
  if (fallbackPointer.mode === "pan") {
    ...
    fallbackCamera.target.x -= dx * panScale * Math.cos(fallbackCamera.yaw);
    fallbackCamera.target.y += dx * panScale * Math.sin(fallbackCamera.yaw);
    fallbackCamera.target.z += dy * panScale;
  } else {
...
function initializeScene() {
  ...
  const addGantryCell = (x, y, span, dynamic = false, rotationRad = 0) => {
    const halfSpanX = span.x * 0.5;
    const halfSpanY = span.y * 0.5;
    const pillarX = Math.max(0.25, halfSpanX - GANTRY_DIMS.uprightSize * 0.5);
    const pillarY = Math.max(0.2, halfSpanY - GANTRY_DIMS.uprightSize * 0.5);
    ...
    const bed = new THREE.Mesh(
      new THREE.BoxGeometry(span.x, span.y, 0.012),
      new THREE.MeshLambertMaterial({ color: 0x375164 })
    );
    bed.position.set(0, 0, 0.006);
    cell.add(bed);
  ...
  gantryArmSystems = [];
  getGantryCellSpecs().forEach(({ x, y, span, dynamic, rotationRad }) => {
    addGantryCell(x, y, span, dynamic, rotationRad);
  });
...
sceneSelect.addEventListener("change", (event) => {
  currentSceneId = event.target.value === "gantry" ? "gantry" : "arm";
  gantryFocusIndex = -1;
  applySceneVisibility();
  if (useCanvasFallback) {
    resetFallbackCamera();
